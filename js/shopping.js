...
function onComputerSelected(computerId) {
	...
	for (var index = 0; index < computers.length; index++) {
...
function onAddToBasket(computerId) {
	...
	setTimeout(function () {
		...
		for (var index = 0; index < basketComputers.length; index++) {
...
function refreshComputersBasket() {
	...
	for (var index = 0; index < basketComputers.length; index++) {
		...
		basketComputersTableBodyElementHtml += ' AZN</td><td><button onclick="devareBasketComputer(' +
			b.computer.id +
...
function devareBasketComputer(computerId) {
	for (var index = 0; index < basketComputers.length; index++) {
		const b = basketComputers[index];
		if (b.computer.id === computerId) {
...
function closeBasket() {
	setTimeout(function () { basketModalElement.style.display = 'none'; }, 2000);
}
...
function closeConfirmOrder() {
	...
	setTimeout(function () { confirmOrderModalElement.style.display = 'none'; }, 1000);
}
...
function onOrderSubmit(event) {
	...
	var userIdList = [];
	for (var index = 0; index < orderObject.basketComputers.length; index++) {
	...
	for (var index = 0; index < userIdList.length; index++) {
		...
		for (var i = 0; i < orderObject.basketComputers.length; i++) {
...
function calculateOrderTotalPrice(order) {
	...
	for (var index = 0; index < order.basketComputers.length; index++) {
...
function onSearchKeyDown(event) {
	if (event.keyCode == 13) {
		...
		setTimeout(function () {
			...
			for (var index = 0; index < computersSelectedGlobal.length; index++) {
			...
			computersElement.style.display = 'block';
			if (findedComputers.length == 0) { computersElement.innerHTML = "<h2 class='not-found'>No computers were found matching this request!</h2>"; }
			allComputersLoaded = false;
		}, 500);
...
function searchCategory(searchInput) {
	...
	categories = [];
	for (var index = 0; index < categoriesGlobal.length; index++) {
